...
Use [msModelSlim](https://gitcode.com/Ascend/msmodelslim) to generate quantized weights.
...
For details about how to install MindIE-LLM, see [Installation Guide](../../install/installing_MindIE.md). The following sections use [Image Installation](../../install/source/image_usage_guide.md) as an example to demonstrate how to run serving inference for the DeepSeek-V3.2 model.
...
Perform the following steps on both servers (for physical machine installation, select **Physical Machine**; for image- or container-based installation, select the corresponding container option) to set environment variables:

```shell
source /usr/local/Ascend/ascend-toolkit/set_env.sh       # Set CANN environment variables (adjust path to your CANN installation)
source /usr/local/Ascend/nnal/atb/set_env.sh             # Set ATB environment variables (adjust path to your ATB installation)
source /usr/local/Ascend/atb-models/set_env.sh           # Set ATB-Models environment variables (adjust path to your atb-llm installation). For whl package, use: /usr/local/lib/python3.11/site-packages/atb_llm/set_env.sh
source /usr/local/Ascend/mindie/set_env.sh               # Set MindIE-LLM environment variables (adjust path to your MindIE installation). For whl package, use: /usr/local/lib/python3.11/site-packages/mindie_llm/set_env.sh
...
- Set `GLOO_SOCKET_IFNAME` to the NIC name corresponding to `MIES_CONTAINER_IP`. You can run the `ifconfig` command to view the NIC list and set this parameter to the corresponding NIC name. For details, see [FAQs](https://gitcode.com/Ascend/MindIE-LLM/blob/v3.1.0/docs/en/faq/faq.md#gloo%E8%BF%9E%E6%8E%A5%E5%A4%B1%E8%B4%A5%E6%8A%A5%E9%94%99%EF%BC%9Aerror-failed-to-connect-errorso_error-connection-refused).
- MindIE-LLM supports building both run packages and whl packages, which differ in installation paths. The provided images install the run package by default. **.run package**: binaries are stored in `/usr/local/Ascend/mindie`, while Python files are placed in the default whl installation directory (e.g., `/usr/local/lib/python3.11/site-packages/mindie_llm`). **.whl package**: both binaries and Python files are installed under the default Python package directory.

#### Setting serving parameters

Perform the following steps on both servers (or containers) to modify the serving parameters. Go to the MindIE-LLM installation directory and edit the serving configuration file.

```shell
cd /usr/local/Ascend/mindie/latest/mindie-service  # For .whl package, the path is /usr/local/lib/python3.11/site-packages/mindie_llm
...
```shell
# Start the service. This step needs to be performed on both servers (hosts or containers).
cd /usr/local/Ascend/mindie/latest/mindie-service  # Depend on the actual installation path
./bin/mindieservice_dameon

# For a .whl package, run mindie_llm_server
```
...
Obtain the MoE EP initialization script by referring to [MindIE Motor CPP](https://gitcode.com/Ascend/MindIE-Motor-CPP/blob/v3.1.0/docs/zh/user_guide/service_deployment/pd_separation_service_deployment.md#%E4%BD%BF%E7%94%A8kubectl%E9%83%A8%E7%BD%B2%E5%8D%95%E6%9C%BApd%E5%88%86%E7%A6%BB%E6%9C%8D%E5%8A%A1%E7%A4%BA%E4%BE%8B).
