...
Quantized MatMul parameters include `input_scale`, `input_offset`, `quant_bias`, and `deq_scale`. `input_scale` and `input_offset` are used to quantize the activations. MatMul uses the quantized activations and weights for computation. `quant_bias` and `deq_scale` are used to dequantize the computation result of MatMul.
