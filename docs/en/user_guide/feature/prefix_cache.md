...
- Prefix cache cannot be enabled simultaneously with SplitFuse and data parallelism.
...
4. Send a request for the first time. The prompt is the first round of questions.
    ...
    To use the Prefix Cache feature, the prompt of the second request must share a certain length of common prefix with the prompt of the first request. Common use scenarios include multi-turn dialogue and few-shot learning.
