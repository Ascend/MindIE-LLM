...
- Check whether the NPU driver and firmware are installed. If information similar to that shown in [Figure 1](#figure1) is displayed, the NPU driver and firmware have been installed. Otherwise, install them by referring to [Table 1](#table1).
    ...
    **Figure 1** Command output<a id="figure1"></a>
    ...
    ![Command output](./figures/command_output.png "Command output")

    **Table 1** Atlas A2 inference products<a id="table1"></a>

    |Product Model |Reference Document|
    |------------|------------|
    | Atlas 800I A2 | Download the [firmware and driver](https://hiascend.com/hardware/firmware-drivers/community). For details about how to install the firmware and driver, see "[Installing the NPU Driver and Firmware](https://www.hiascend.com/document/detail/en/CANNCommunityEdition/latest/softwareinst/instg/instg_0005.html?OS=openEuler&InstallType=local)" in *CANN Software Installation (Commercial Edition)*. |
...
1. Download the weight file first. The following uses Qwen2-7B as an example. Download its weight file from [https://huggingface.co/Qwen/Qwen2-7B/tree/main](https://huggingface.co/Qwen/Qwen2-7B/tree/main) and upload it to any directory, for example, `/home/weight/qwen2-7b`, on the server.
...
> [!NOTE]
>
> - In some image versions, MindIE LLM is installed as a .whl package. You can check its path with `pip show mindie_llm`. Newer and older versions use the .run package installation method, which differs primarily in installation path and service startup approach.
> - With .whl installation: MindIE LLM is located at `{site_packages}/mindie_llm/`, and ATB Models at `/usr/local/Ascend/atb_llm`.
> - With .run package installation: MindIE-LLM is located at `/usr/local/Ascend/mindie/latest/mindie-llm/`, and ATB Models at `/usr/local/Ascend/atb-models`.
...
1. Start the container after the image is downloaded.
    ...
    ```
     ...
     > [!NOTE]
     > - `{IMAGE_ID}` is the image ID. After the image is built, run `docker images` to find the ID and replace it in the command above.
     > - For the `--device` flag, set the mount permission to `rwm` instead of the more restrictive `rw` or `r`, for the following reasons:
     >   - On Atlas 800I A2 Inference Servers, `rw` permission allows container entry and `npu-smi` to view NPU usage, and MindIE services run normally. However, if the mounted NPU (e.g., `davinci0` for `npu0`) is already occupied by another task, `npu-smi` will print an error and MindIE tasks will fail (including `torch.npu.set_device()`).
     >   - On Atlas 800I A3 SuperPoD Servers, `rw` permission causes `npu-smi` to print an error upon container entry and prevents MindIE tasks from running (including `torch.npu.set_device()`).
    ...
    **Table 1** Parameter description
    ...
    |--shm-size|Specifies the size of the shared memory (**/dev/shm**) of a specified container. You can set the size as required. `500g` is an example value. For multimodal understanding models, if the maximum service concurrency is high, setting this parameter to at least 500 GB is recommended.<br>The value cannot exceed the size of the remaining physical memory of the host. You can run the `free -h` command to view the size of the remaining physical memory. When data parallelism (DP) is enabled, the shared memory size (**shm-size**) must be adjusted proportionally as the DP value grows beyond 1.<ul><li>For a DP value of 2, set **shm-size** to at least 2 GB.</li><li>For a DP value of 4, set **shm-size** to at least 3 GB.</li><li>For a DP value of 8, set **shm-size** to at least 5 GB.</li><li>For a DP value of 16, set **shm-size** to at least 9 GB.</li></ul>|
...
2. Set environment variables. <a id="setup_env"></a>
...
3. Set serving parameters.
    ...
    d. Set the `config.json` file permissions to `640`.
...
4. Start the service.
    ...
    ```
     ...
     >
     > [!NOTE]
     > - If you are using a MindIE LLM image installed via a partial .whl package, you can start the service by running the `mindie-llm-server` command from any location outside the MindIE LLM installation path.

5. Send a request.

    For the serving API interfaces, refer to the [Serving API Usage Guide](https://www.hiascend.com/document/detail/en/mindie/310/mindiellm/llmdev/mindie_llm/user_guide/user_manual/service_APIs_usage_guidance.md).
...
> [!NOTE]
>
>- Before the accuracy and performance tests, open another window to access the container and set environment variables by referring to [3](#setup_env).
...
1. Install AISBench.
    ...
    > [!NOTE]
    >
    > - The MindIE image includes the AISBench tool pre-installed, so the installation steps above are not required. You can check its installation path with `pip show ais_bench_benchmark`. In the following instructions, this pre-installed version is referred to as the "image-integrated version," while the version installed via `git clone` is called the "git-installed version."
    > - In the following instructions, `{ais_bench_path}` denotes the AISBench installation path. For the image-integrated version, it is typically located at `{site-packages}/ais_bench`; for the git-installed version, it is the `ais_bench` subdirectory in the cloned repository root.

2. Prepare a dataset.

    Using gsm8k as an example: download the dataset by clicking [gsm8k](https://opencompass.oss-cn-shanghai.aliyuncs.com/datasets/data/gsm8k.zip), then extract the archive and place the `gsm8k` folder under `{ais_bench_path}/datasets` in the tool root directory.

3. Configure the `{ais_bench_path}/benchmark/configs/models/vllm_api/vllm_api_stream_chat.py` file. The following is an example:
    ...
    models = [
        dict(
            ...
            path="/home/weight/qwen2-7b",                    # Absolute path to the model serialized vocabulary file, which is generally the path to the model weight folder.
...
1. Install the AISBench tool (see [Accuracy Test](#accuracy-test) for details).
2. Prepare a dataset.
...
4. Run the following command to start the serving performance test:
    ...
    Performance test results focus primarily on TTFT, TPOT, Request Throughput, and Output Token Throughput. For detailed parameter descriptions, refer to "Table 2: Performance Test Result Metric Comparison" in "Auxiliary Tools" > "[Performance/Accuracy Test Tools](https://www.hiascend.com/document/detail/en/mindie/310/mindiellm/llmdev/mindie_motor_cpp/user_guide/service_oriented_optimization_tool/performance_accuracy_test_tool.md)" in *MindIE Motor CPP Development Guide*.
