...
4. (Optional) Enable HTTPS authentication (that is, set `httpsEnabled` to `true`).
   ...
   a. Use the certificate import script to import certificates. [Table 1](#table1) describes the certificate information.
    ...
    > - For details about the certificate import script of the service, see "Auxiliary Tools" > "MindIE Service Tools" > <a href="https://www.hiascend.com/document/detail/zh/mindie/22RC1/mindieservice/servicedev/mindie_service0312.html">CertTools</a>" in *MindIE Motor Development Guide*.
    > - If the certificate import times out, see [Starting the haveged Service](../install/faq_and_appendixes/starting_the_haveged_service.md).
      ...
      | Encrypted password of the service certificate private key | {MindIE installation directory}/latest/mindie-service/security/pass/  | This parameter is optional.                                           |
...
4. Set parameters in the container as required.
   ...
   b. Press `i` to enter edit mode, set `"multiNodesInferEnabled"=true` to enable multi-node inference, and modify the parameters in Table 1 as needed. For details, see [Configuration Parameters (Serving)](service_parameter_configuration.md)..

      Table 1 Multi-node inference configuration
...
5. (Optional) If gRPC two-way authentication is enabled (that is, interNodeTLSEnabled is set to true),
    ...
    a. Use the certificate management tool to import the certificate. For details about how to use the certificate management tool, see "Auxiliary Tools" > "MindIE Service Tools" > "[CertTools](https://gitcode.com/Ascend/MindIE-Motor/blob/v3.0.0/docs/zh/user_guide/service_deployment/single_machine_service_deployment.md)" in *MindIE Motor Development Guide*. [Table 2](#table2) describes the certificate files.
    ...
    Table 2 Certificate file information <a id="table2"></a>
    ...
    | Encrypted password of the service certificate private key| mindie-service/security/pass/       | Required.                                                      |
...
6. (Optional) Enable HTTPS authentication (that is, set `httpsEnabled` to `true`).
    ...
    a. Use the certificate import script of the Service to import the certificate. For details about the certificate information, see [Table 3] (#table3).

    > [!NOTE]NOTE
    > - When three-plane isolation is enabled for HTTPS, you are advised not to use the same security certificate for the HTTPS service plane and management plane. Using the same security certificate can cause high network security risks.
    > - You are advised not to use the same security certificate for HTTPS and gRPC. Using the same certificate may lead to significant network security risks.
    > - When importing certificates, ensure that the script permissions required for CA certificates, service certificates, private key certificates, and CRL certificates are 600, 400, and 600, respectively.
    > - For details about the certificate import script of the service, "Auxiliary Tools" > "MindIE Service Tools" > "CertTools" in *MindIE Motor Development Guide*
    > - If the certificate import times out, see [Starting the haveged Service](../install/faq_and_appendixes/starting_the_haveged_service.md).

      Table 3 Certificate files <a id="table3"></a>
...
9. Configure the environment variables for running the ATB Models.
...
10. Set the environment variables `RANK_TABLE_FILE` and `MIES_CONTAINER_IP` (e.g., using the rank table example from the [sample ranktable file](https://gitcode.com/Ascend/MindIE-Motor/blob/v3.0.0/docs/zh/user_guide/service_deployment/pd_separation_service_deployment.md); see Table 4 for details).
...
11. Start the service. This operation must be performed in containers on both the Master and Slave nodes.
