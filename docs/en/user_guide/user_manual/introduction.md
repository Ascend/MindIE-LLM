...
![](./figures/mindie_llm_architecture_diagram.png)
    ...
    - Engine: Orchestrates the collaboration among Scheduler, Executor, Worker, and other components to enable request inference across multiple scenarios through component synergy.

    - Scheduler: Batches multiple requests during either Prefill or Decode phases within a single DP domain, maximizing computation and communication utilization.

    - Block manager: Manages KV resources within a DP domain, with pool support and location awareness for offloaded KV data.

    - Executor: Dispatches scheduled information to the Text Generator module and supports task distribution across nodes and cards.
