...
|MINDIE_LLM_HOME_PATH|MindIE LLM home path.|Path parameter|`/usr/local/Ascend/mindie/latest/mindie-service`|
|MINDIE_LLM_RECOMPUTE_THRESHOLD|Recomputation threshold in MindIE LLM.|[0,1]|0.5|
|PYTORCH_INSTALL_PATH|Installation path of the third-party component torch. You can run the following command to obtain the path: `python3 -c 'import torch, os; print(os.path.dirname(os.path.abspath(torch.__file__)))'`|N/A|N/A|
...
|ASCEND_GLOBAL_EVENT_ENABLE|Whether to enable event logging for applications.|**0**: Event log disabled. <br>**1**: enabled|0|
...
For more ATB-related environment variables, see "[Environment Variables](https://www.hiascend.com/document/detail/en/CANNCommunityEdition/latest/API/ascendtb/ascendtb_0032.html)" in the *CANN ATB Development Guide*.
