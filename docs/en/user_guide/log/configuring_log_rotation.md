...
The size of each log file ranges from 1MB to 500MB, and the default value is 20MB. The number of log files that can be written by each process ranges from 1 to 64, and the default value is 10.
...
[Example 1] Set the size of each log file of all  MindIE  components to 500MB:

```bash
export MINDIE_LOG_ROTATE="-fs 500"
```

[Example 2] Set the maximum size of the  MindIE Torch  log file to 40MB and allow each process to write one log file:
