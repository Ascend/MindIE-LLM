...
> [!NOTE]
>
> - The Server supports bidirectional HTTPS authentication to authenticate HTTPS requests initiated by clients. When HTTPS is enabled, to encrypt the password of the server key, the random number generation algorithm used by the encryption tool KMC requires the haveged component to supplement entropy. Perform the following operations on all nodes where the Server is installed.
