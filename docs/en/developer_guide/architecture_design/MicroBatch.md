...
2. Set serving parameters. Add the `"micro_batch"` field to the Server `config.json` file (highlighted in bold below). For parameter descriptions, refer to Table 1 Micro Batch parameter: `models` in `ModelConfig`. For serving parameters, see "Configuration Parameters (Serving)". Below is an example configuration:
