...
This document explains how to build the MindIE-LLM `.whl` package from source and install and run it.

## Environment Setup

### Installation via Image

For MindIE images, refer to [Installation via Image](../user_guide/install/source/image_usage_guide.md#obtaining-the-mindie-image).

### Installation on Containers or Physical Machines
...
1. Install Python. MindIE-LLM supports **Python 3.10** and **Python 3.11**.
...
2. Run the following commands to clone the source code repository:
...
3. Run the following command to compile the third-party dependencies:
...
5. Compile and generate the `.whl` software package of MindIE-LLM.
    ...
    * After the compilation, the temporary directory `build`, binary directory `output`, and debug symbol table `llm_debug_symbols` are generated.

6. Run the following command to install MindIE-LLM:
...
7. Compile the `.whl` software package of ATB_Models.
    ...
    > **Note**: If Python 3.10 is used for compilation, torch 2.9.0 and TorchNPU 2.9.0 must be used. Otherwise, the \_bz2 module will be missing, causing compilation failure.

8. Run the following command to install ATB_Models:

    ```bash
    pip install atb_llm*.whl
    ```

9. (Optional) Configure the operating environment variables.
...
For upgrade instructions, see [Upgrade](../user_guide/install/source/upgrade.md).

## Uninstallation

For uninstallation instructions, see [Uninstallation](../user_guide/install/source/uninstallation.md).
