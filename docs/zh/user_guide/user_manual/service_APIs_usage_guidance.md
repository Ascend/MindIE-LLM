...
Server提供EndPoint模块对推理服务协议和接口封装，兼容Triton/OpenAI/TGI/vLLM等第三方框架接口。使用单节点安装模式安装Server之后，用户使用客户端（Linux curl命令，Postman工具等）发送HTTP/HTTPS请求，即可调用EndPoint提供的接口。
...
**表 1** 服务状态查询API（内部接口的查询类接口） <a id="table1"></a>
...
|health|GET|/health|服务健康检查。|<ul><li>TGI</li><li>vLLM</li></ul>|
...
|服务管控指标查询接口（普罗格式）|GET|/metrics|查询推理服务的相关服务管控指标|原生|
