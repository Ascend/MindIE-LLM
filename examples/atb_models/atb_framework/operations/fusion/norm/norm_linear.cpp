...
bool UseNormQuant(const NormLinearParam<NormParamType> &param) {
    if (param.fusionLinearParam.quantType ==
            LinearQuantType::LINEAR_W8A8_DEQUANT ||
        param.fusionLinearParam.quantType ==
            LinearQuantType::LINEAR_W8A8_SC_DEQUANT ||
        (param.fusionLinearParam.quantType ==
             LinearQuantType::LINEAR_W8A8_DYNAMIC_DEQUANT &&
         param.enableAddNorm) ||
        (param.fusionLinearParam.quantType ==
             LinearQuantType::LINEAR_W4A8_DYNAMIC_DEQUANT &&
         param.enableAddNorm)) {
        return true;
    } else {
        return false;
...
std::map<std::string, std::vector<std::string>> GetInTensorCandidates() {
    std::map<std::string, std::vector<std::string>> normInTensorCandidates = {
        {"default",
         {"in_input", "in_norm_weight", "in_norm_bias", "in_norm_new_weight",
          "in_norm_new_bias", "in_linear_weight", "in_scale", "in_offset",
          "in_descale", "in_bias", "in_compress_idx"}},
        {"add_norm", {"in_residual_input"}},
        {"add_rmsnorm_quant", {"in_scale_fill", "in_offset_fill"}},
        {"lora",
         {"in_seq_len_cum_sum", "in_linear_lora_a", "in_linear_lora_b"}},
...
std::map<std::string, uint32_t> ConstructNormTensorMap(
    const NormLinearParam<NormParamType> &param, uint32_t &inTensorNum,
    ...
    if (!param.skipNorm) {
        AddTensorToList(normIntermediateTensorCandidates, "default",
                        intermediateTensorList);
    }

    // 添加add norm特性的Tensor、添加AddRmsNormQuant特性的Tensor
    if (param.enableAddNorm) {
        AddTensorToList(normInTensorCandidates, "add_rmsnorm_quant",
                        inTensorList);
    ...
    if (param.fusionLinearParam.supportLora) {
        if (param.fusionLinearParam.useImMask) {
            AddTensorToList(normInTensorCandidates, "lora_with_mask",
                            inTensorList);
        }
    ...
    if (UseAddRmsNormQuant(param)) {
        AddTensorToList(normIntermediateTensorCandidates, "addrmsnormquant",
                        intermediateTensorList);
    }
    if (UseAddRmsNormDynamicQuant(param)) {
        AddTensorToList(normIntermediateTensorCandidates,
                        "addrmsnormdynamicquant", intermediateTensorList);
    }
    if (param.enableModelConfuscation) {
        if (param.modelObfuscationParallelInfo.worldSize > 1) {
            AddTensorToList(normIntermediateTensorCandidates, "pmcc_multi",
                            intermediateTensorList);
        } else {
            AddTensorToList(normIntermediateTensorCandidates, "pmcc",
                            intermediateTensorList);
        }
    }
...
template <typename NormParamType>
int64_t InsertNorm(atb::GraphParam &opGraph,
    ...
    aclNNAddNormQuantMatmulParam.epsilon =
        param.normParamType.normParam.epsilon;
    AclNNAddNormDynamicQuantMatmulParam aclNNAddNormDynamicQuantMatmulParam;
    aclNNAddNormDynamicQuantMatmulParam.epsilon =
        param.normParamType.normParam.epsilon;
    atb::Node normNode;
    if (param.enableAddNorm) {
        normNode.inTensorIds.push_back(
            GetTensorIdx(tensorMap, "in_residual_input"));
    }
    ...
    if (useAddRmsNormQuant || useAddRmsNormDynamicQuant) {
        ...
        if (useAddRmsNormDynamicQuant) {
            normNode.outTensorIds.push_back(
                GetTensorIdx(tensorMap, "scale1_out"));
            normNode.outTensorIds.push_back(
                GetTensorIdx(tensorMap, "scale2_out"));
        }
    ...
    if (useNormQuant) {  // activation quant
        ...
        normNode.inTensorIds.push_back(
            param.isAntiOutlier ? GetTensorIdx(tensorMap, "in_norm_new_weight")
                                : GetTensorIdx(tensorMap, "in_norm_weight"));
        if (!useAddRmsNormQuant && !useAddRmsNormDynamicQuant) {
            normNode.inTensorIds.push_back(
                param.isAntiOutlier
                    ? GetTensorIdx(tensorMap, "in_norm_new_bias")
                    : GetTensorIdx(tensorMap, "in_norm_bias"));
        }
        if (useAddRmsNormQuant) {
            normNode.inTensorIds.push_back(
                GetTensorIdx(tensorMap, "in_scale_fill"));
            normNode.inTensorIds.push_back(
                GetTensorIdx(tensorMap, "in_offset_fill"));
        } else if (!useAddRmsNormDynamicQuant) {
            normNode.inTensorIds.push_back(GetTensorIdx(tensorMap, "in_scale"));
            normNode.inTensorIds.push_back(
                GetTensorIdx(tensorMap, "in_offset"));
        }
        if (useAddRmsNormQuant) {  // aclnn接口
            normNode.operation =
                new atb_speed::common::AddRmsNormQuantOperation(
            ...
            normNode.operation =
                new atb_speed::common::AddRmsNormDynamicQuantOperation(
                    "AddRmsNormDynamicQuantOperation",
                    aclNNAddNormDynamicQuantMatmulParam);
        } else {  // ATB接口
            CHECK_OPERATION_STATUS_RETURN(atb::CreateOperation(
                param.normQuantParamType, &normNode.operation));
        }
    } else {  // activation no quant
        CHECK_OPERATION_STATUS_RETURN(
            atb::CreateOperation(param.normParamType, &normNode.operation));
        normNode.inTensorIds.push_back(GetTensorIdx(tensorMap, "in_input"));
        normNode.inTensorIds.push_back(
            param.isAntiOutlier ? GetTensorIdx(tensorMap, "in_norm_new_weight")
                                : GetTensorIdx(tensorMap, "in_norm_weight"));
...
template <typename NormParamType>
atb::Status InsertObfuscationCalculate(
    atb::GraphParam &opGraph, const NormLinearParam<NormParamType> &param,
    std::map<std::string, uint32_t> &tensorMap) {
    ...
    if (isMultiRank) {
        ...
        sliceParam.offsets = {
            0, CheckIntMulOverFlow(rank, param.hiddenSizePerRank)};
        sliceParam.size = {-1, param.hiddenSizePerRank};
        sliceNode.inTensorIds.push_back(
            GetTensorIdx(tensorMap, "intermediate_norm"));
        sliceNode.outTensorIds.push_back(
            GetTensorIdx(tensorMap, "intermediate_norm_per_rank"));
        CHECK_OPERATION_STATUS_RETURN(
    ...
    obfCalNode.inTensorIds.push_back(
        GetTensorIdx(tensorMap, isMultiRank ? "intermediate_norm_per_rank"
                                            : "intermediate_norm"));
    obfCalNode.outTensorIds.push_back(
        GetTensorIdx(tensorMap, isMultiRank ? "intermediate_pmcc_gather"
                                            : "intermediate_pmcc"));
    obfCalNode.operation = new atb_speed::common::ObfuscationCalculateOperation(
        "ObfCalculateOperation", obfCalParam);
    ...
    if (isMultiRank) {
        ...
        allGatherParam.rankTableFile =
            param.modelObfuscationParallelInfo.rankTableFile;
        allGatherParam.hcclComm = param.modelObfuscationParallelInfo.hcommInfo;
        allGatherParam.commDomain =
            param.modelObfuscationParallelInfo.commDomain;
        CHECK_OPERATION_STATUS_RETURN(
            atb::CreateOperation(allGatherParam, &allGatherNode.operation));
        allGatherNode.inTensorIds = {
            GetTensorIdx(tensorMap, "intermediate_pmcc_gather")};
        allGatherNode.outTensorIds = {
            GetTensorIdx(tensorMap, "intermediate_pmcc_gather_out")};
        ...
        transposeNode.outTensorIds = {
            GetTensorIdx(tensorMap, "intermediate_pmcc")};
...
template <typename NormParamType>
atb::Status NormLinear(const NormLinearParam<NormParamType> &param,
                       atb::Operation **operation) {
    atb::GraphParam opGraph;
    opGraph.name = "NormLinear";

    std::map<std::string, uint32_t> tensorMap =
        ConstructNormTensorMap(param, opGraph.inTensorNum, opGraph.outTensorNum,
    ...
    bool normIsPostNorm =
        static_cast<int>(param.normParamType.layerType) ==
            static_cast<int>(
                atb::infer::RmsNormParam::RmsNormType::RMS_NORM_POSTNORM) ||
        static_cast<int>(param.normParamType.layerType) ==
            static_cast<int>(
                atb::infer::LayerNormParam::LayerNormType::LAYER_NORM_POSTNORM);
    bool normQuantIsPostNorm =
    ...
    if (param.enableModelConfuscation) {
        CHECK_OPERATION_STATUS_RETURN(
            InsertObfuscationCalculate(opGraph, param, tensorMap));
    }
    ...
    // 则设为LinearQuantType::LINEAR_W8A8_DYNAMIC_QUANT
    if (linearParam.quantType == LinearQuantType::LINEAR_W8A8_DYNAMIC_DEQUANT &&
        !param.enableAddNorm) {
        linearParam.quantType = LinearQuantType::LINEAR_W8A8_DYNAMIC_QUANT;
    }
    // if LINEAR_W4A8_DYNAMIC_DEQUANT and AddRmsNormQuant is false, then set to
    // LINEAR_W4A8_DYNAMIC_QUANT,
    if (linearParam.quantType == LinearQuantType::LINEAR_W4A8_DYNAMIC_DEQUANT &&
        !param.enableAddNorm) {
    ...
    std::vector<std::string> linearInTensor = {
        param.skipNorm ? "in_input"
                       : (param.enableModelConfuscation ? "intermediate_pmcc"
                                                        : "intermediate_norm"),
        "in_linear_weight",
        "in_scale",
        "in_offset",
    ...
    if (param.enableModelConfuscation &&
        param.modelObfuscationParallelInfo.worldSize > 1) {
        linearNode.inTensorReshapeFuncs.resize(linearNode.inTensorIds.size());
        linearNode.inTensorReshapeFuncs[0] = [=](const atb::Dims &oldShape,
                                                 atb::Dims &newShape) {
...
bool IsFloatLinearType(const int &linearType, const int &linearDesc) {
    return linearType == atb_speed::common::LinearType::FP ||
...
bool IsW4A16QuantType(const int &packQuantType, const int &linearDesc) {
    return packQuantType == atb_speed::common::ALL_W4A16 ||
...
bool IsW8A16QuantType(const int &packQuantType, const int &linearDesc) {
    return packQuantType == atb_speed::common::ALL_W8A16 ||
...
bool IsW4A8QuantType(const int &packQuantType, const int &linearDesc) {
    return packQuantType == atb_speed::common::ALL_W4A8 ||
...
LinearQuantType GetW4A8QuantType(bool hasNorm) {
    return hasNorm ? LinearQuantType::LINEAR_W4A8_DYNAMIC_DEQUANT
                   : LinearQuantType::LINEAR_W4A8_DYNAMIC_QUANT;
}

bool IsW8A8SCQuantType(const int &packQuantType, const int &linearDesc) {
    return packQuantType == atb_speed::common::ALL_W8A8SC ||
...
LinearQuantType GetW8A8SCQuantType(bool hasNorm) {
    return hasNorm ? LinearQuantType::LINEAR_W8A8_SC_DEQUANT
                   : LinearQuantType::LINEAR_W8A8_SC_QUANT;
}
...
LinearQuantType GetLinearQuantType(const int &packQuantType,
                                   const int &linearType, bool hasNorm,
                                   const int &linearDesc, bool supportLora) {
...
template bool UseNormQuant(
    const NormLinearParam<atb::infer::RmsNormParam> &param);
template bool UseAddRmsNormQuant(
    const NormLinearParam<atb::infer::RmsNormParam> &param);
template bool UseAddRmsNormDynamicQuant(
    const NormLinearParam<atb::infer::RmsNormParam> &param);
...
template int64_t InsertNorm(
    ...
    const NormLinearParam<atb::infer::RmsNormParam> &param,
    std::map<std::string, uint32_t> &tensorMap);
template atb::Status InsertObfuscationCalculate(
    atb::GraphParam &opGraph,
    const NormLinearParam<atb::infer::RmsNormParam> &param,
    std::map<std::string, uint32_t> &tensorMap);
template atb::Status NormLinear(
    const NormLinearParam<atb::infer::RmsNormParam> &param,
    atb::Operation **operation);
