    ...
    std::map<std::string, atb::SVector<std::string>> mlpInTensorCandidates = {
        {"default",
         {"in_input",           "in_norm_weight",      "in_norm_bias",
          ...
          "in_bias_1",          "in_compress_idx_1",   "in_weight_down",
          "in_scale_down",      "in_offset_down",      "in_descale_down",
          "in_bias_down",       "in_compress_idx_down"}},
        ...
        {"lora",
         {"in_seq_len_cum_sum", "in_lora_a_0", "in_lora_b_0", "in_lora_a_1",
          "in_lora_b_1", "in_down_lora_a", "in_down_lora_b"}},
        {"reduce_quant",
...
atb::SVector<std::string> ConstructMlpInTensorList(
    ...
    if (param.enableAddNorm) {
        AddTensorToList(mlpInTensorCandidates, "add_rmsnorm_quant",
                        inTensorList);
    ...
    if (param.supportLora) {
        if (param.useImMask) {
            AddTensorToList(mlpInTensorCandidates, "lora_with_mask",
                            inTensorList);
        }
...
template <typename NormParamType>
atb::SVector<std::string> ConstructMlpOutTensorList(
    const MlpParam<NormParamType> &param) {
...
void SetGateUpNormLinearParam(
    ...
    gateUpNormLinearParam.fusionLinearParam.quantType = GetLinearQuantType(
        param.packQuantType, param.layerLinearQuantType[GATE_LINEAR_INDEX],
        param.enableNormQuantOp, param.layerLinearDescs[GATE_LINEAR_INDEX],
        param.supportLora);
    ...
    gateUpNormLinearParam.fusionLinearParam.transposeType =
        param.layerLinearTransposeType[GATE_LINEAR_INDEX];
    gateUpNormLinearParam.fusionLinearParam.quantGroupSize =
    ...
    gateUpNormLinearParam.fusionLinearParam.enableFlashComm =
...
atb::Status AddMlpNormLinearGateUp(const MlpParam<NormParamType> &param,
    ...
    CHECK_OPERATION_STATUS_RETURN(
        NormLinear<NormParamType>(gateUpNormLinearParam, &normLinearGateUpOp));

    atb::SVector<std::string> gateUpInTensorNames = {
        "in_input",           "in_norm_weight",    "in_norm_bias",
        "in_norm_new_weight", "in_norm_new_bias",  "in_weight_0",
        "in_scale_0",         "in_offset_0",       "in_descale_0",
        "in_bias_0",          "in_compress_idx_0",
    };
    if (param.enableAddNorm) {
    ...
    graphBuilder->AddOperation(normLinearGateUpOp, gateUpInTensorNames,
                               gateUpOutTensorNames);
    return atb::NO_ERROR;
}

template <typename NormParamType>
void SetUpNormLinearParam(
    ...
    upNormLinearParam.fusionLinearParam.quantType = GetLinearQuantType(
        param.packQuantType, param.layerLinearQuantType[UP_LINEAR_INDEX],
        param.enableNormQuantOp, param.layerLinearDescs[UP_LINEAR_INDEX],
        param.supportLora);
    ...
    upNormLinearParam.fusionLinearParam.transposeType =
        param.layerLinearTransposeType[UP_LINEAR_INDEX];
...
atb::Status AddMlpNormLinearUp(const MlpParam<NormParamType> &param,
    ...
    CHECK_OPERATION_STATUS_RETURN(
        NormLinear<NormParamType>(upNormLinearParam, &normLinearUpOp));

    atb::SVector<std::string> upInTensorNames = {
        "in_input",           "in_norm_weight",    "in_norm_bias",
        "in_norm_new_weight", "in_norm_new_bias",  "in_weight_1",
        "in_scale_1",         "in_offset_1",       "in_descale_1",
        "in_bias_1",          "in_compress_idx_1",
    };
    if (param.supportLora) {
        if (param.useImMask) {
    ...
    graphBuilder->AddOperation(normLinearUpOp, upInTensorNames,
                               {"intermediate_up"});
    return atb::NO_ERROR;
}
...
atb::Status AddPrefetchWeight(const MlpParam<NormParamType> &param,
    ...
    CHECK_OPERATION_STATUS_RETURN(
        atb_speed::EventManager::GetInstance().RecordEvent(
            computeRecordNodeOp, atb_speed::EventAction::PUSH,
            atb_speed::common::CMO_COMPUTE));
    graphBuilder->AddOperation(computeRecordNodeOp,
                               computeRecordNodeInTensorIds,
                               computeRecordNodeOutTensorIds);
    ...
    CHECK_OPERATION_STATUS_RETURN(
        atb_speed::EventManager::GetInstance().WaitEvent(
            commWaitNodeOp, atb_speed::EventAction::POP,
            atb_speed::common::CMO_COMPUTE));
    atb::SetExecuteStreamId(commWaitNodeOp, 1);
    graphBuilder->AddOperation(commWaitNodeOp, commWaitNodeInTensorIds,
                               commWaitNodeOutTensorIds);
    ...
    graphBuilder->AddOperation(cmoNodeOp, cmoNodeInTensorIds,
                               cmoNodeOutTensorIds);
    return atb::NO_ERROR;
}
...
atb::Status AddMlpSplit(atb::GraphOpBuilder *&graphBuilder) {
    ...
    graphBuilder->AddOperation(splitOp, {"intermediate_gate_up"},
...
atb::Status AddMlpSwiGLUConcat(atb::GraphOpBuilder *&graphBuilder) {
    ...
    graphBuilder->AddOperation(concatOp,
                               {"intermediate_gate", "intermediate_up"},
...
atb::Status AddMlpMul(atb::GraphOpBuilder *&graphBuilder) {
    ...
    elewiseParam.elewiseType =
        atb::infer::ElewiseParam::ElewiseType::ELEWISE_MUL;
    CHECK_OPERATION_STATUS_RETURN(atb::CreateOperation(elewiseParam, &mulOp));

    graphBuilder->AddOperation(
        mulOp, {"intermediate_activation_out", "intermediate_up"},
        {"intermediate_activation_out"});
    return atb::NO_ERROR;
}

template <typename NormParamType>
atb::Status AddMlpActivation(const MlpParam<NormParamType> &param,
                             atb::GraphOpBuilder *&graphBuilder) {
    if (param.mlpPackType == MlpPackType::GATE_UP_WEIGHT_PACK) {
    ...
    CHECK_OPERATION_STATUS_RETURN(
        atb::CreateOperation(param.activationParam, &activationOp));
    graphBuilder->AddOperation(activationOp,
                               {param.mlpPackType == MlpPackType::UP_WEIGHT_ONLY
                                    ? "intermediate_up"
                                    : "intermediate_gate"},
                               {"intermediate_activation_out"});
...
template <typename NormParamType>
atb::Status AddMlpEdgeActivation(const MlpParam<NormParamType> &param,
                                 atb::GraphOpBuilder *&graphBuilder) {
    if (param.mlpPackType == MlpPackType::GATE_UP_WEIGHT_PACK) {
        CHECK_OPERATION_STATUS_RETURN(AddMlpSplit(graphBuilder));
    }
    ...
    activationParam.activationType =
        atb::infer::ActivationType::ACTIVATION_SIGMOID;
    CHECK_OPERATION_STATUS_RETURN(
        atb::CreateOperation(activationParam, &sigmoidOp));
    graphBuilder->AddOperation(sigmoidOp, {"intermediate_gate"},
                               {"intermediate_activation_out"});
    ...
    elewiseParam.elewiseType =
        atb::infer::ElewiseParam::ElewiseType::ELEWISE_MUL;
    CHECK_OPERATION_STATUS_RETURN(
        atb::CreateOperation(elewiseParam, &sigmoidMulOp));
    graphBuilder->AddOperation(
        sigmoidMulOp, {"intermediate_gate", "intermediate_activation_out"},
        {"intermediate_activation_out"});
...
template <typename NormParamType>
atb::Status AddMlpSwiGLUActivation(const MlpParam<NormParamType> &param,
                                   atb::GraphOpBuilder *&graphBuilder) {
    ...
    CHECK_OPERATION_STATUS_RETURN(
        atb::CreateOperation(param.activationParam, &activationOp));
    graphBuilder->AddOperation(activationOp, {"intermediate_gate_up"},
                               {"intermediate_activation_out"});
    return atb::NO_ERROR;
}

template <typename NormParamType>
void SetDownLinearParallelParam(
    const MlpParam<NormParamType> &param,
    atb_speed::common::LinearParallelParam &downLinearParallelParam) {
    if (param.enableFlashComm) {
        downLinearParallelParam.parallelType =
            atb_speed::common::REDUCE_SCATTER;
    } else {
    ...
    downLinearParallelParam.fusionLinearParam.quantType = GetLinearQuantType(
        ...
        param.layerLinearDescs[DOWN_LINEAR_INDEX], param.supportLora);
    bool downIsQuant = IsLinearDescQuant(param, DOWN_LINEAR_INDEX);
    if (param.enableSwigluQuant && downIsQuant) {
        downLinearParallelParam.fusionLinearParam.enableSwigluQuant =
        ...
        if (downLinearParallelParam.fusionLinearParam.quantType ==
            ...
            downLinearParallelParam.fusionLinearParam.quantType =
                atb_speed::common::LinearQuantType::LINEAR_W8A8_DEQUANT;
                   ...
                   atb_speed::common::LinearQuantType::
    ...
    downLinearParallelParam.biasAfterSync =
        ...
        param.downHasBias;
    downLinearParallelParam.fusionLinearParam.hasBias =
        param.downHasBias && !downLinearParallelParam.biasAfterSync;
    ...
    downLinearParallelParam.fusionLinearParam.loraEnableGMM =
        param.loraEnableGMM;
    downLinearParallelParam.fusionLinearParam.transposeType =
        param.layerLinearTransposeType[DOWN_LINEAR_INDEX];
    downLinearParallelParam.fusionLinearParam.quantGroupSize =
        param.quantGroupSize;
    downLinearParallelParam.fusionLinearParam.matmulBackend =
        param.matmulBackend;
    downLinearParallelParam.tensorParallelInfo =
        param.downLinearTensorParallelInfo;
    downLinearParallelParam.supportLcoc = param.supportLcoc;
    downLinearParallelParam.enableMC2 = param.enableMC2;
}

template <typename NormParamType>
atb::Status AddMlpLinearDown(const MlpParam<NormParamType> &param,
                             atb::GraphOpBuilder *&graphBuilder) {
    atb::Operation *linearDownOp = nullptr;
    atb_speed::common::LinearParallelParam downLinearParallelParam;
    SetDownLinearParallelParam(param, downLinearParallelParam);
    CHECK_OPERATION_STATUS_RETURN(
        LinearParallel(downLinearParallelParam, &linearDownOp));
    ...
    if (downLinearParallelParam.parallelType ==
        atb_speed::common::REDUCE_SCATTER) {
...
template <typename NormParamType>
atb::Status AddDequantSwigluQuantNode(const MlpParam<NormParamType> &param,
                                      atb::GraphOpBuilder *&graphBuilder) {
    ...
    if (gateUpIsQuant && UseQuantBatchMatmul(linearParam)) {
        inTensorNames.push_back(
            "in_descale_0");  // 1: weightScaleOptional, fp32
    ...
    atb::SVector<std::string> outTensorNames = {
        "intermediate_activation_out", "intermediate_swiglu_dynamic_scale"};
    aclnnParam.inTensorsNum = static_cast<int>(inTensorNames.size());
    atb::Operation *dequantSwigluQuantOp =
        new atb_speed::common::DequantSwigluQuantOperation(
            "aclNNDequantSwigluQuantNode", aclnnParam);
    graphBuilder->AddOperation(dequantSwigluQuantOp, inTensorNames,
                               outTensorNames);
    return atb::NO_ERROR;
}

template <typename NormParamType>
atb::Status Mlp(const MlpParam<NormParamType> &param,
    ...
    if (DestroyGraphOpBuilder(graphOpBuilder) != atb::NO_ERROR) {
        ATB_SPEED_LOG_WARN(
            "Destroy graph builder failed. This may leads to memory leak, "
            "please "
            "check");
    }
    return res;
...
template <typename NormParamType>
atb::Status MlpSwiGLU(const MlpParam<NormParamType> &param,
                      atb::Operation **operation) {
    atb::GraphOpBuilder *graphOpBuilder = nullptr;
    CHECK_OPERATION_STATUS_RETURN(CreateGraphOpBuilder(&graphOpBuilder));
...
atb::Status CheckMlpParam(const MlpParam<NormParamType> &param) {
    if (param.layerLinearDescs.size() != 0 &&
        CheckParamVectorSize(param.layerLinearDescs, DOWN_LINEAR_INDEX + 1) !=
            atb::NO_ERROR) {
        ATB_SPEED_LOG_ERROR(
    ...
    if (param.layerLinearQuantType.size() != 0 &&
        CheckParamVectorSize(param.layerLinearQuantType,
                             DOWN_LINEAR_INDEX + 1) != atb::NO_ERROR) {
        ATB_SPEED_LOG_ERROR(
    ...
    if (CheckParamVectorSize(param.layerLinearTransposeType,
                             DOWN_LINEAR_INDEX + 1) != atb::NO_ERROR) {
        ATB_SPEED_LOG_ERROR(
            "The size of param.layerLinearTransposeType is wrong, please "
            "check");
        return atb::ERROR_INVALID_PARAM;
    }
    return atb::NO_ERROR;
...
template <typename NormParamType>
atb::Status CreateMlp(const MlpParam<NormParamType> &param,
    ...
    atb::InferShapeFunc inferShapeFunc =
        [param](const atb::SVector<atb::TensorDesc> &inTensorDescs,
    ...
    CHECK_OPERATION_STATUS_RETURN(graphOpBuilder->Init(
        graphName, inferShapeFunc, ConstructMlpInTensorList(param),
        ConstructMlpOutTensorList(param)));

    // Gate Up
    CHECK_OPERATION_STATUS_RETURN(
        AddMlpNormLinearGateUp(param, isAntiOutlier, graphOpBuilder));
    if (param.mlpPackType == MlpPackType::GATE_UP_WEIGHT_NO_PACK) {
        CHECK_OPERATION_STATUS_RETURN(
            AddMlpNormLinearUp(param, isAntiOutlier, graphOpBuilder));
    }
    // prefetch weight
    ...
    if (param.isEdgeHardware) {
        CHECK_OPERATION_STATUS_RETURN(
            AddMlpEdgeActivation(param, graphOpBuilder));
    } else if (isSwiGLU) {
        bool downIsQuant = IsLinearDescQuant(param, DOWN_LINEAR_INDEX);
        if (param.enableSwigluQuant && downIsQuant) {
            CHECK_OPERATION_STATUS_RETURN(
                AddDequantSwigluQuantNode(param, graphOpBuilder));
        } else {
            CHECK_OPERATION_STATUS_RETURN(
                AddMlpSwiGLUActivation(param, graphOpBuilder));
        }
    } else {
...
template <typename NormParamType>
bool IsLinearDescQuant(const MlpParam<NormParamType> &param,
...
template bool IsLinearDescQuant(const MlpParam<atb::infer::RmsNormParam> &param,
                                const uint64_t index);

template bool IsLinearDescQuant(
    const MlpParam<atb::infer::LayerNormParam> &param, const uint64_t index);

template void SetDownLinearParallelParam(
...
template atb::Status CheckMlpParam(
    const MlpParam<atb::infer::RmsNormParam> &param);

template atb::Status CheckMlpParam(
    const MlpParam<atb::infer::LayerNormParam> &param);

template atb::SVector<std::string> ConstructMlpInTensorList(
    const MlpParam<atb::infer::RmsNormParam> &param);

template atb::SVector<std::string> ConstructMlpInTensorList(
    const MlpParam<atb::infer::LayerNormParam> &param);

template atb::SVector<std::string> ConstructMlpOutTensorList(
    const MlpParam<atb::infer::RmsNormParam> &param);
...
template void SetGateUpNormLinearParam(
    atb_speed::common::NormLinearParam<atb::infer::RmsNormParam>
        &gateUpNormLinearParam,
    const MlpParam<atb::infer::RmsNormParam> &param, bool isAntiOutlier);

template void SetGateUpNormLinearParam(
    atb_speed::common::NormLinearParam<atb::infer::LayerNormParam>
        &gateUpNormLinearParam,
    const MlpParam<atb::infer::LayerNormParam> &param, bool isAntiOutlier);

template atb::Status AddMlpNormLinearGateUp(
    const MlpParam<atb::infer::RmsNormParam> &param, bool isAntiOutlier,
    atb::GraphOpBuilder *&graphBuilder);
...
template atb::Status AddMlpLinearDown(
    const MlpParam<atb::infer::RmsNormParam> &param,
    atb::GraphOpBuilder *&graphBuilder);
...
template atb::Status AddPrefetchWeight(
    const MlpParam<atb::infer::RmsNormParam> &param,
    atb::GraphOpBuilder *&graphBuilder);
...
template atb::Status AddMlpNormLinearUp(
    const MlpParam<atb::infer::RmsNormParam> &param, bool isAntiOutlier,
    atb::GraphOpBuilder *&graphBuilder);
...
template atb::Status AddMlpActivation(
    const MlpParam<atb::infer::RmsNormParam> &param,
    atb::GraphOpBuilder *&graphBuilder);
...
template atb::Status AddMlpEdgeActivation(
    const MlpParam<atb::infer::RmsNormParam> &param,
    atb::GraphOpBuilder *&graphBuilder);

template atb::Status AddMlpSwiGLUActivation(
    const MlpParam<atb::infer::RmsNormParam> &param,
    atb::GraphOpBuilder *&graphBuilder);
...
template atb::Status AddDequantSwigluQuantNode(
    const MlpParam<atb::infer::RmsNormParam> &param,
    atb::GraphOpBuilder *&graphBuilder);
...
template atb::Status Mlp(const MlpParam<atb::infer::RmsNormParam> &param,
                         atb::Operation **operation);

template atb::Status Mlp(const MlpParam<atb::infer::LayerNormParam> &param,
                         atb::Operation **operation);

template atb::Status MlpSwiGLU(const MlpParam<atb::infer::RmsNormParam> &param,
                               atb::Operation **operation);

template atb::Status MlpSwiGLU(
    const MlpParam<atb::infer::LayerNormParam> &param,
    atb::Operation **operation);

template atb::Status CreateMlp(const MlpParam<atb::infer::RmsNormParam> &param,
                               atb::GraphOpBuilder *&graphOpBuilder,
                               atb::Operation **operation, bool isSwiGLU);

template atb::Status CreateMlp(
    const MlpParam<atb::infer::LayerNormParam> &param,
    atb::GraphOpBuilder *&graphOpBuilder, atb::Operation **operation,
    bool isSwiGLU);
