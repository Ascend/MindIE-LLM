    ...
    std::map<std::string, std::vector<std::string>> qkvInTensorCandidates = {
        ...
        {"lora",
         {"in_seq_len_cum_sum", "in_qkv_lora_a_0", "in_qkv_lora_b_0",
          "in_qkv_lora_a_1", "in_qkv_lora_b_1", "in_qkv_lora_a_2",
          "in_qkv_lora_b_2"}},
        ...
        {"flash_comm",
         {"send_counts", "sdispls", "send_count", "recv_counts", "rdispls",
          "recv_count", "fake_ag_shape"}},
    };
    return qkvInTensorCandidates;
}

std::map<std::string, std::vector<std::string>>
GetQKVIntermediateTensorCandidates() {
    std::map<std::string, std::vector<std::string>>
        qkvIntermediateTensorCandidates = {
            {"qkv_pack", {"intermediate_qkv"}},
            {"qk_norm",
             {"intermediate_q", "intermediate_k", "intermediate_q_rstd_out",
              "intermediate_k_rstd_out"}},
            {"add_norm", {"out_add"}},
        };
...
std::map<std::string, uint32_t> ConstructQKVTensorMap(
    ...
    std::vector<int> qkvLinearIndex = {Q_LINEAR_INDEX, K_LINEAR_INDEX,
                                       V_LINEAR_INDEX};
    bool isPack =
        CheckPack(param.packQuantType, param.layerLinearDescs, qkvLinearIndex);

    // 添加默认的Tensor
    AddTensorToList(qkvInTensorCandidates, "default", inTensorList);
    // 添加AddRmsNormQuant特性的Tensor
    if (param.enableAddNorm) {
        AddTensorToList(qkvInTensorCandidates, "add_rmsnorm_quant",
                        inTensorList);
    ...
    if (param.supportLora) {
        if (param.useImMask) {
            AddTensorToList(qkvInTensorCandidates, "lora_with_mask",
                            inTensorList);
        }
    ...
    if (isPack && !param.enableRopeQuantKvcache) {
        AddTensorToList(qkvIntermediateTensorCandidates, "qkv_pack",
                        intermediateTensorList);
        if (param.useQKNorm) {
            AddTensorToList(qkvIntermediateTensorCandidates, "qk_norm",
                            intermediateTensorList);
...
atb::Status AddQNormLinearNode(const FusionAttentionParam<NormParamType> &param,
    ...
    if (param.layerLinearQuantType.size() != 0 &&
        CheckParamVectorSize(param.layerLinearQuantType, Q_LINEAR_INDEX + 1) !=
            atb::NO_ERROR) {
        ATB_SPEED_LOG_ERROR(
    ...
    qNormLinearParam.fusionLinearParam.transposeType =
        param.layerLinearTransposeType[0];
    ...
    qNormLinearParam.fusionLinearParam.flashCommParallelInfo.rank =
        param.selfOutLinearTensorParallelInfo.rank;
    ...
    qNormLinearParam.modelObfuscationParallelInfo =
        param.selfOutLinearTensorParallelInfo;
    CHECK_OPERATION_STATUS_RETURN(NormLinear<NormParamType>(
        qNormLinearParam, &qNormLinearNode.operation));
...
template <typename NormParamType>
atb::Status AddSplitQKVNode(const FusionAttentionParam<NormParamType> &param,
                            atb::GraphParam &opGraph,
                            std::map<std::string, uint32_t> &tensorMap) {
    ...
    if (param.splitWithStride) {
        ...
        splitQKVParam = {
            (param.isFA ? 2 : 1),
            3,
            {CheckIntMulOverFlow(param.selfAttentionParam.headNum,
                                 param.headDim),
             CheckIntMulOverFlow(param.selfAttentionParam.kvHeadNum,
                                 param.headDim),
             CheckIntMulOverFlow(param.selfAttentionParam.kvHeadNum,
                                 param.headDim)}};
    }
    CHECK_OPERATION_STATUS_RETURN(
        atb::CreateOperation(splitQKVParam, &splitQKVNode.operation));
    splitQKVNode.inTensorIds = {GetTensorIdx(tensorMap, "intermediate_qkv")};
    splitQKVNode.outTensorIds = {GetTensorIdxList(
        tensorMap, {param.useQKNorm ? "intermediate_q" : "out_q",
                    param.useQKNorm ? "intermediate_k" : "out_k", "out_v"})};
    if (param.splitWithStride) {
        splitQKVNode.inTensorReshapeFuncs.resize(
            splitQKVNode.inTensorIds.size());
        splitQKVNode.inTensorReshapeFuncs[0] = [=](const atb::Dims &oldShape,
                                                   atb::Dims &newShape) {
            InternlmV2QKVSplit(
                oldShape, newShape, param.selfAttentionParam.headNum,
...
template <typename NormParamType>
atb::Status AddSplitMixedQKVNode(
    const FusionAttentionParam<NormParamType> &param, atb::GraphParam &opGraph,
    std::map<std::string, uint32_t> &tensorMap) {
    ...
    CHECK_OPERATION_STATUS_RETURN(
        atb::CreateOperation(splitMixedQKVParam, &splitMixedQKVNode.operation));
    splitMixedQKVNode.inTensorIds = {
        GetTensorIdx(tensorMap, "intermediate_qkv")};
    splitMixedQKVNode.outTensorIds =
        GetTensorIdxList(tensorMap, {"out_q", "out_k", "out_v"});
    if (param.splitWithStride) {
        splitMixedQKVNode.inTensorReshapeFuncs.resize(
            splitMixedQKVNode.inTensorIds.size());
        splitMixedQKVNode.inTensorReshapeFuncs[0] =
            [=](const atb::Dims &oldShape, atb::Dims &newShape) {
                size_t dim = 0;
                newShape.dims[dim++] =
                    oldShape.dims[0];  // PA ntokens | FA batch
                if (param.isFA) {
                    newShape.dims[dim++] = oldShape.dims[1];  // FA seqlen
                }
                newShape.dims[dim++] =
                    param.selfAttentionParam.headNum;  // headNum
...
atb::Status AddKNormLinearNode(const FusionAttentionParam<NormParamType> &param,
    ...
    kNormLinearParam.fusionLinearParam.transposeType =
        param.layerLinearTransposeType[1];
    ...
    CHECK_OPERATION_STATUS_RETURN(NormLinear<NormParamType>(
        kNormLinearParam, &kNormLinearNode.operation));
...
atb::Status AddQKNormNode(const FusionAttentionParam<NormParamType> &param,
    ...
    qNormNode.inTensorIds.push_back(
        GetTensorIdx(tensorMap, "in_q_norm_weight"));
    qNormNode.outTensorIds.push_back(GetTensorIdx(tensorMap, "out_q"));
    qNormNode.outTensorIds.push_back(
        GetTensorIdx(tensorMap, "intermediate_q_rstd_out"));
    qNormNode.inTensorReshapeFuncs.resize(qNormNode.inTensorIds.size());
    qNormNode.inTensorReshapeFuncs[0] = [=](const atb::Dims &oldShape,
                                            atb::Dims &newShape) {
        newShape.dimNum = 3;                  // 3: 新的shape维度为3
        newShape.dims[0] = oldShape.dims[0];  // 0: bs * seq_len
        newShape.dims[1] =
            oldShape.dims[1] / param.headDim;  // 1: 128 q headDim
        newShape.dims[2] = param.headDim;      // 128: headDim
    };
    qNormNode.operation = new atb_speed::common::RmsNormOperation(
        "QRmsNormNode", param.normParamType.normParam.epsilon);

    atb::Node kNormNode;
    kNormNode.inTensorIds.push_back(GetTensorIdx(tensorMap, "intermediate_k"));
    kNormNode.inTensorIds.push_back(
        GetTensorIdx(tensorMap, "in_k_norm_weight"));
    kNormNode.outTensorIds.push_back(GetTensorIdx(tensorMap, "out_k"));
    kNormNode.outTensorIds.push_back(
        GetTensorIdx(tensorMap, "intermediate_k_rstd_out"));
    kNormNode.inTensorReshapeFuncs.resize(kNormNode.inTensorIds.size());
    kNormNode.inTensorReshapeFuncs[0] = [=](const atb::Dims &oldShape,
                                            atb::Dims &newShape) {
        newShape.dimNum = 3;                  // 3: 新的shape维度为3
        newShape.dims[0] = oldShape.dims[0];  // 0: bs * seq_len
        newShape.dims[1] =
    ...
    kNormNode.operation = new atb_speed::common::RmsNormOperation(
        "KRmsNormNode", param.normParamType.normParam.epsilon);
...
atb::Status AddVNormLinearNode(const FusionAttentionParam<NormParamType> &param,
    ...
    vNormLinearParam.fusionLinearParam.transposeType =
        param.layerLinearTransposeType[V_LINEAR_INDEX];
    ...
    CHECK_OPERATION_STATUS_RETURN(NormLinear<NormParamType>(
        vNormLinearParam, &vNormLinearNode.operation));
...
void QKVLinearSplitInferShapeFunc(
    const FusionAttentionParam<NormParamType> &param, atb::GraphParam &opGraph,
    uint32_t inQKVInputIdx, uint32_t inResidualAddInputIdx,
    uint32_t inFakeAgShapeIdx) {
    if (param.isFA) {
        opGraph.inferShapeFunc =
            [inQKVInputIdx, inResidualAddInputIdx, param](
                ...
                outTensorDescs.at(0).shape.dims[0] =
                    inTensorDescs.at(inQKVInputIdx)
                        .shape.dims[0];  // 0, 0, 0: batch size
                outTensorDescs.at(0).shape.dims[1] =
                    inTensorDescs.at(inQKVInputIdx)
                        .shape.dims[1];  // 0, 1, 1: seq len
                outTensorDescs.at(0).shape.dims[2] =
                    param.selfAttentionParam.headNum;  // 0, 2: headNum
                outTensorDescs.at(0).shape.dims[3] =
                    param.headDim;  // 0, 3: headDim

                outTensorDescs.at(1) = outTensorDescs.at(0);
                outTensorDescs.at(1).shape.dims[2] =
                    param.selfAttentionParam.kvHeadNum;  // 0, 2: kvHeadNum

                outTensorDescs.at(2) = outTensorDescs.at(
                    1);  // 2: 第2个输出tensor的描述和第1个输出tensor的描述一致
                if (param.enableAddNorm) {
                    outTensorDescs.at(3) = inTensorDescs.at(
                        inResidualAddInputIdx);  // 3: AddNorm融合有第3个输出
                }
                return atb::NO_ERROR;
            };
    } else {
        opGraph.inferShapeFunc =
            [inQKVInputIdx, inResidualAddInputIdx, param, inFakeAgShapeIdx](
                const atb::SVector<atb::TensorDesc> &inTensorDescs,
                atb::SVector<atb::TensorDesc> &outTensorDescs) {
                outTensorDescs.at(0) = inTensorDescs.at(inQKVInputIdx);
                ...
                if (param.enableFlashComm) {
                    outTensorDescs.at(0).shape.dims[0] =
                        inTensorDescs.at(inFakeAgShapeIdx).shape.dims[0];
                } else {
                    outTensorDescs.at(0).shape.dims[0] =
                        inTensorDescs.at(inQKVInputIdx)
                            .shape.dims[0];  // 0, 0, 0: batch size * seq len
                }
                outTensorDescs.at(0).shape.dims[1] =
                    param.selfAttentionParam.headNum;  // 0, 1: headNum
                outTensorDescs.at(0).shape.dims[2] =
                    param.headDim;  // 0, 2: headDim

                outTensorDescs.at(1) = outTensorDescs.at(0);
                outTensorDescs.at(1).shape.dims[1] =
                    param.selfAttentionParam.kvHeadNum;  // 0, 1: kvHeadNum
...
template <typename NormParamType>
atb::Status QKVLinearSplit(const FusionAttentionParam<NormParamType> &param,
                           atb::Operation **operation) {
    if (param.layerLinearDescs.size() != 0 &&
        CheckParamVectorSize(param.layerLinearDescs, V_LINEAR_INDEX + 1) !=
            atb::NO_ERROR) {
        ATB_SPEED_LOG_ERROR(
    ...
    std::map<std::string, uint32_t> tensorMap =
        ConstructQKVTensorMap(param, opGraph.inTensorNum, opGraph.outTensorNum,
    ...
    ATB_SPEED_LOG_DEBUG("qkv opGraph.internalTensorNum "
                        << opGraph.internalTensorNum);
    ...
    CHECK_OPERATION_STATUS_RETURN(
        AddQNormLinearNode(param, opGraph, tensorMap, isAntiOutlier, isPack));

    if (!param.enableRopeQuantKvcache) {
        if (isPack && param.isGroupedQueryAttention) {  // Split GQA
            CHECK_OPERATION_STATUS_RETURN(
                AddSplitQKVNode(param, opGraph, tensorMap));
            if (param.useQKNorm) {
                CHECK_OPERATION_STATUS_RETURN(
                    AddQKNormNode(param, opGraph, tensorMap));
            }
        } else if (isPack && !param.isGroupedQueryAttention) {  // Split MHA
            CHECK_OPERATION_STATUS_RETURN(
                AddSplitMixedQKVNode(param, opGraph, tensorMap));
        } else {  // isPack: false
            if (param.layerLinearQuantType.size() != 0 &&
                CheckParamVectorSize(param.layerLinearQuantType,
                                     V_LINEAR_INDEX + 1) != atb::NO_ERROR) {
                ATB_SPEED_LOG_ERROR(
                    "The size of param.layerLinearQuantType is wrong, please "
                    "check");
                return atb::ERROR_INVALID_PARAM;
            }
            if (CheckParamVectorSize(param.layerLinearTransposeType,
                                     V_LINEAR_INDEX + 1) != atb::NO_ERROR) {
                ATB_SPEED_LOG_ERROR(
                    "The size of param.layerLinearTransposeType is wrong, "
                    "please "
                    "check");
                return atb::ERROR_INVALID_PARAM;
            }
            CHECK_OPERATION_STATUS_RETURN(
                AddKNormLinearNode(param, opGraph, tensorMap, isAntiOutlier));
            CHECK_OPERATION_STATUS_RETURN(
                AddVNormLinearNode(param, opGraph, tensorMap, isAntiOutlier));
        }

        uint32_t inQKVInputIdx = GetTensorIdx(tensorMap, "in_qkv_input");
        uint32_t inResidualAddInputIdx =
            GetTensorIdx(tensorMap, "in_residual_add");
        uint32_t inFakeAgShapeIdx = GetTensorIdx(tensorMap, "fake_ag_shape");
        QKVLinearSplitInferShapeFunc(param, opGraph, inQKVInputIdx,
                                     inResidualAddInputIdx, inFakeAgShapeIdx);
    }
...
template void QKVLinearSplitInferShapeFunc(
    const FusionAttentionParam<atb::infer::RmsNormParam> &param,
