...
atb::Status ConstructAttentionQuantTensorMap(
    ...
    if (param.pageAttentionParam.quantType ==
        ...
        if (param.pageAttentionParam.hasQuantOffset) {
            AddTensorToList(attnInTensorCandidates, "kv_quant_offset",
                            inTensorList);
        }
    }
...
std::map<std::string, uint32_t> ConstructTensorMap(
    ...
    auto attnIntermediateTensorCandidates =
        GetAttnIntermediateTensorCandidates();
    ...
    if (param.enableAddNorm) {
        AddTensorToList(attnInTensorCandidates, "add_rmsnorm_quant",
                        inTensorList);
    }
    ...
    if (param.pageAttentionParam.compressType ==
        ...
        AddTensorToList(attnInTensorCandidates, "compress_head_rope",
                        inTensorList);
    }
    // 添加并行解码特性的Tensor
    ...
    if (param.supportLora) {
        if (param.useImMask) {
            AddTensorToList(attnInTensorCandidates, "lora_with_mask",
                            inTensorList);
        }
...
template <typename NormParamType>
atb::Status AddFAttnQKVLinearSplitNode(
...
atb::Status AddFAttnRopeNode(const FusionAttentionParam<NormParamType> &param,
    ...
    if (param.ropeBackend == atb_speed::common::OpBackend::ATB) {
        ...
        ropeNode.inTensorIds = {GetTensorIdx(tensorMap, "intermediate_q"),
                                GetTensorIdx(tensorMap, "intermediate_k"),
                                GetTensorIdx(tensorMap, "in_cos_embed"),
                                GetTensorIdx(tensorMap, "in_sin_embed")};
    }
...
template <typename NormParamType>
atb::Status AddKVValueQuantNode(
    const FusionAttentionParam<NormParamType> &param, atb::GraphParam &opGraph,
...
atb::Status AddQKVQuantNode(atb::GraphParam &opGraph,
                            std::map<std::string, uint32_t> &tensorMap,
                            std::string nodeType) {
...
template <typename NormParamType>
atb::Status AddSelfOutLinearParallelNode(
    const FusionAttentionParam<NormParamType> &param, atb::GraphParam &opGraph,
    std::map<std::string, uint32_t> &tensorMap) {
    ...
    selfOutLinearParam.fusionLinearParam.quantType = GetLinearQuantType(
        ...
        param.layerLinearDescs[DENSE_LINEAR_INDEX], param.supportLora);
    ...
    selfOutLinearParam.tensorParallelInfo =
    ...
    CHECK_OPERATION_STATUS_RETURN(LinearParallel(
        selfOutLinearParam, &selfOutLinearParallelNode.operation));
...
atb::Status Attention(const FusionAttentionParam<NormParamType> &param,
    ...
    if (param.enableRopeQuantKvcache) {
        ...
        if (param.enableQScale) {
            CHECK_OPERATION_STATUS_RETURN(
                AddQScaleNode(param, opGraph, tensorMap));
        }
...
template atb::Status AddSelfOutLinearParallelNode(
    const FusionAttentionParam<atb::infer::RmsNormParam> &param,
    atb::GraphParam &opGraph, std::map<std::string, uint32_t> &tensorMap);
template atb::Status AddQScaleNode(
    const FusionAttentionParam<atb::infer::RmsNormParam> &param,
    atb::GraphParam &opGraph, std::map<std::string, uint32_t> &tensorMap);
