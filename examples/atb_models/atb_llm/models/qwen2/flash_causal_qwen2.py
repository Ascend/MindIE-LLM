...
from ..base.graph_manager.layerwise_combined_graph_wrapper import (
    LayerwiseCombinedATBGraphWrapper,
)
...
class FlashQwen2ForCausalLM(FlashForCausalLM):
    def __init__(self, config, weights, **kwargs):
        ...
        self.enable_rope_quant_kvcache = (
            self.config.quantization_config.kv_quant_type is not None
        ...
        self.prealloc_weight_mem_on_npu = kwargs.get(
            "prealloc_weight_mem_on_npu", False
        )
        if self.layerwise_disaggregated:
            ...
            if self.layerwise.split_type == DistributedType.CLOUD:
                ...
                self.layerwise.load_list = [
                    i for i in range(0, self.layerwise.edge_start_layer_count)
                ]
                edge_start_layer_count = (
                    self.config.num_hidden_layers - self.layerwise.edge_end_layer_count
                )
                self.layerwise.load_list.extend(
                    [
                        i
                        for i in range(
                            edge_start_layer_count, self.config.num_hidden_layers
                        )
                    ]
                )
            ...
            self.model = Qwen2Model(
                config, model_prefix, quant_config=kwargs.get("quant_config")
            )
        else:
            self.transformer = FlashQwenModel(
                config,
        ...
        qwen2_5_has_yarn = (
            ...
            and self.config.rope_scaling.type == "yarn"
        )
        qwen3_has_yarn = (
            hasattr(self.config, "rope_scaling")
            and self.config.rope_scaling.rope_type == "yarn"
        )
        if qwen2_5_has_yarn or qwen3_has_yarn:
            ...
            if self.config.rope_scaling.factor is None:
                raise ValueError(
                    "config.rope_scaling.factor must be set in config.json"
                )
            if self.config.rope_scaling.factor <= 1:
                self.mscale = self.attention_factor
            else:
                self.mscale = float(
                    (0.1 * math.log(self.config.rope_scaling.factor) + 1.0)
        ...
        if self.layerwise_disaggregated:
            ...
            self.graph_manager = ATBGraphManager(
                prefill_graph, decode_graph, LayerwiseCombinedATBGraphWrapper
            )
        else:
            self.graph_manager = ATBGraphManager()
        ...
        self.enable_swiglu_quant = (
            not self.soc_info.need_nz
            and self.mempool_type != MemPoolType.ASYNC_WRITE
            and not has_lora
        )
        self.flash_comm_modifier = FlashCommModifier(
            weights, self.hidden_size, self.flash_comm_gate(weights)
        )
    ...
    def get_weights(self, quantize_type: QuantType = None):
        ...
        weight_wrapper = WeightWrapper(
            self.soc_info, self.tp_rank, attn_wrapper, mlp_wrapper, **kwargs
        )
        ...
        for i in range(self.num_layers):
            ...
            if self.mempool_type != MemPoolType.ASYNC_WRITE and (
                ...
                weight_wrapper.register_layer_addrmsnormquant(
                    layer, attn_wrapper, mlp_wrapper, self.quantize
                )
    ...
    def get_model_weights(self, quant_type: QuantType = None):
        ...
        weights.extend(
            self.model.norm.get_weights_for_atb_graph(padding=False)
        )  # length: 1
        weights.extend(
            self.lm_head.get_weights_for_atb_graph(padding=False)
        )  # length: 1
    ...
    def get_layer_weights(self, layer: CustomLayer, quant_type: QuantType = None):
        ...
        weights_per_layer.extend(
            layer.input_layernorm.get_weights_for_atb_graph()
        )  # length: 4

        qkv_proj_adapter = layer.self_attn.qkv_proj
        weights_per_layer.extend(
            qkv_proj_adapter.get_weights_for_atb_graph(quant_type=quant_type)
        )  # length: 18
        linear_descs_per_layer.extend(qkv_proj_adapter.get_linear_descs())  # length: 7
        weight_transpose_type_per_layer.extend(
            qkv_proj_adapter.get_weight_transpose_type()
        )  # length: 7

        o_proj_adapter = layer.self_attn.o_proj
        weights_per_layer.extend(
            o_proj_adapter.get_weights_for_atb_graph(quant_type=quant_type)
        )  # length: 6
        linear_descs_per_layer.extend(o_proj_adapter.get_linear_descs())  # length: 7
        weight_transpose_type_per_layer.extend(
            o_proj_adapter.get_weight_transpose_type()
        )  # length: 7

        weights_per_layer.extend(
            layer.post_attention_layernorm.get_weights_for_atb_graph()
        )  # length: 4

        gate_up_proj_adapter = layer.mlp.gate_up_proj
        weights_per_layer.extend(
            gate_up_proj_adapter.get_weights_for_atb_graph(quant_type=quant_type)
        )  # length: 12
        linear_descs_per_layer.extend(
            gate_up_proj_adapter.get_linear_descs()
        )  # length: 7
        weight_transpose_type_per_layer.extend(
            gate_up_proj_adapter.get_weight_transpose_type()
        )  # length: 7
        ...
        weight_transpose_type_per_layer.extend(
            down_proj_adapter.get_weight_transpose_type()
        )  # length: 7

        if self.config.use_qk_norm:
            weights_per_layer.extend(
                layer.self_attn.q_norm.get_weights_for_atb_graph(padding=False)
            )  # length: 1
            weights_per_layer.extend(
                layer.self_attn.k_norm.get_weights_for_atb_graph(padding=False)
            )  # length: 1
    ...
    def get_layerwsie_ascend_param(
        self, ascend_params, mode, linear_has_bias, wrapper: WeightWrapper
    ):
        ...
        if mode == LwdLayerStatus.EDGE_START_LAYER:
            modify_ascend_params[LINEAR_HAS_BIAS] = (
                linear_has_bias * self.layerwise.edge_start_layer_count
            )
            ...
            modify_ascend_params[LWD_END_ID] = (
                self.config.num_hidden_layers - self.layerwise.edge_end_layer_count
            )
        elif mode == LwdLayerStatus.EDGE_END_LAYER:
            modify_ascend_params[LINEAR_HAS_BIAS] = (
                linear_has_bias * self.layerwise.edge_end_layer_count
            )
            modify_ascend_params[LWD_START_ID] = (
                self.config.num_hidden_layers - self.layerwise.edge_end_layer_count
            )
            modify_ascend_params[LWD_END_ID] = self.config.num_hidden_layers
        modify_ascend_params["numHiddenLayers"] = (
            modify_ascend_params[LWD_END_ID] - modify_ascend_params[LWD_START_ID]
        )

        return modify_ascend_params

    def get_layerwise_weights(
        self, mode, quantize_type: QuantType = None, layer_no=None, is_prefill=False
    ):
        quantize_type = self.quantize if quantize_type is None else quantize_type
        attn_wrapper = AttnWrapper(
            norm_name="ln_1",
        ...
        if mode == LwdLayerStatus.EDGE_START_LAYER:
            ...
            end_layer = (
                self.layerwise.load_list.index(self.config.num_hidden_layers - 1) + 1
            )
    ...
    def init_ascend_weight(self):
        ...
        if not self.layerwise_disaggregated:
            if self.prealloc_weight_mem_on_npu:
                if self.quantize == QuantType.W8A8_PDMIX:
                    self.ascend_weight, linear_descs_configs, linear_transpose_types = (
                        self.get_model_weights(quant_type=QuantType.W8A8_DYNAMIC)
                    )
                    self.decode_weight, _, _ = self.get_model_weights(
                ...
                if self.quantize == QuantType.W8A8_PDMIX:
                    weight_wrapper = self.get_weights(
                        quantize_type=QuantType.W8A8_DYNAMIC
                    )
                    decode_weight_wrapper = self.get_weights(
                        quantize_type=QuantType.W8A8
                    )
            ...
            if self.layerwise.split_type == DistributedType.CLOUD:
                ...
                for i in range(
                    ...
                    self.layerwise.weight_wrappers.append(
                        self.get_layerwise_weights(
                            mode=LwdLayerStatus.CLOUD_MIDDLE_LAYER,
                            layer_no=i,
                            is_prefill=True,
                        )
                    )
                ...
                weight_wrapper_head = self.get_layerwise_weights(
                    mode=LwdLayerStatus.EDGE_START_LAYER
                )
                weight_wrapper_tail = self.get_layerwise_weights(
                    mode=LwdLayerStatus.EDGE_END_LAYER
                )
        ...
        if not self.prealloc_weight_mem_on_npu and not self.layerwise_disaggregated:
            ...
            not self.prealloc_weight_mem_on_npu
            and self.layerwise.split_type == DistributedType.EDGE
        ):
        ...
        if self.quantize == QuantType.W8A8_PDMIX:
            linear_descs_configs = [
                [
                    linear_desc
                    if linear_desc != LinearTypeV2.W8A8_PDMIX
                    else LinearTypeV2.W8A8_DYNAMIC
            ...
            decode_linear_descs_configs = [
                [
                    linear_desc
                    if linear_desc != LinearTypeV2.W8A8_DYNAMIC
                    else LinearTypeV2.W8A8
                    for linear_desc in linear_descs
                ]
                for linear_descs in linear_descs_configs
            ]
        ...
        acl_param_dict = {
            ...
            "matmulBackend": OpBackend.ACLNN
            if self.aclnn_matmul_backend
            else OpBackend.ATB,
            ...
            "enableQScale": (
                self.config.transformers_version == "4.43.1"
                or self.config.transformers_version == "4.44.0"
            )
        ...
        encoder_param = {
            **acl_param_dict,
            ...
            and not self.omni_attention_enable,
        }
        decoder_param = {
            **acl_param_dict,
            "isPrefill": False,
        ...
        if not self.layerwise_disaggregated:
            ...
            self.graph_manager.set_param(
                CPP_QWEN_MODEL_CLASS_NAME, encoder_param, specified_params
            )
            ...
            if self.layerwise.split_type == DistributedType.EDGE:
                ...
                self.graph_manager.register_graph(SplitFuseGraphWrapper())
                self.graph_manager.set_param(
                    CPP_QWEN_MODEL_CLASS_NAME, layerwise_encoder_param, specified_params
                )
                ...
                for layer in range(
                    ...
                    encoder_internal_param = self.get_layerwsie_ascend_param(
                        ...
                        1,
                        linear_has_bias,
                        self.layerwise.weight_wrappers[layer],
                    )
                    encoder_internal_param[LWD_START_ID] = (
                        self.layerwise.edge_start_layer_count + layer
                    )
                    encoder_internal_param[LWD_END_ID] = (
                        self.layerwise.edge_start_layer_count + layer + 1
                    )
                    encoder_internal_param["numHiddenLayers"] = 1
                    encoder_internal_param[LINEAR_HAS_BIAS] = linear_has_bias
                    encoder_internal_param["reuseEmbedTable"] = (
                        self.long_seq_enable and layer != 0
                    )
                    encoder_internal_param["outputEmbedTable"] = (
                        self.long_seq_enable and layer == 0
                    )
                ...
                self.graph_manager.register_graph(SplitFuseGraphWrapper())
                self.graph_manager.set_param(
                    CPP_QWEN_MODEL_CLASS_NAME,
                    layerwise_encoder_params,
                    specified_params,
                )
                self.graph_manager.set_weight(
                    layerwise_encode_weights, specified_weights
                )
    ...
    def flash_comm_gate(self, weights) -> bool:
        ...
        if (
            not self.soc_info.is_support_hccs()
            and self.soc_info.soc_version in A2_SOCS + A3_SOCS
        ):
            return False
        if weights.quant_desc is None:
            ...
            fallback_count = sum(
                ...
                and value == "FLOAT"
                and ("mlp.c_proj" in key or "mlp.down_proj" in key)
            )
    ...
    def prepare_inputs_for_ascend(
        ...
        **kwargs,
        ...
        if self.long_seq_enable:
            self.rotary_embedding.yarn_scaling_rotary_embedding(
                self.config, self.device, max_seq_len
            )
        else:
        ...
        if attention_mask is None:
            if is_prefill:
                attention_mask = self.attn_mask.get_rope_prefill_mask(
                    self.max_base_len, self.dtype, self.device
                )
            else:
                attention_mask = self.attn_mask.get_rope_decode_mask(
        ...
        acl_operation_inputs_ = [
            ...
            if is_prefill
            else self.placeholder,  # IN_TENSOR_LOGTIS_INDICES
        ]  # 0-11

        self.acl_param = {"seqLen": input_lengths.tolist()}

        self.long_seq_modifier.modify_inputs(
            acl_operation_inputs_, self.rotary_embedding, position_ids
        )
        self.qlen_modifier.modify_inputs(
            ...
            enable_prefill_pa=False
            if self.inference_mode is None
            else self.inference_mode.enable_prefill_pa,
            enable_splitfuse_pa=not self.soc_info.is_300i(),
            **kwargs,
        )
        self.lora_modifier.modify_inputs(
            acl_operation_inputs_, kwargs.get("adapter_ids"), input_lengths, is_prefill
        )
        self.flash_comm_modifier.modify_inputs(
            acl_operation_inputs_, is_prefill, self.acl_param
        )
    ...
    def execute_ascend_operator(self, acl_inputs, acl_param, is_prefill, **kwargs):
        ...
        runtime_mempool_type = (
            self.mempool_type if self.warmup_is_end else MemPoolType.DISABLED
        )
        ...
        try:
            acl_model_out = self.layerwise_modifier.process_out(
                acl_model_out, is_prefill=is_prefill, **kwargs
            )
    ...
    def execute_dap_ascend_operator(
        self, acl_inputs: list, acl_param: str, is_prefill: bool
    ) -> torch.Tensor:
        ...
        if len(acl_model_out) != 2:
            raise RuntimeError(
                "Number of output tensors is not equal to the expected value."
            )
    ...
    def init_kvcache(self, kv_cache):
        ...
        if kcache_diff or vcache_diff:
            ...
            if self.soc_info.need_nz:
                k_caches = [
                    torch_npu.npu_format_cast_(k_cache, 29) for k_cache in k_caches
                ]
                v_caches = [
                    torch_npu.npu_format_cast_(v_cache, 29) for v_cache in v_caches
                ]
            ...
            if self.layerwise_disaggregated:
                if self.layerwise.split_type == DistributedType.EDGE:
                    ...
                    self.graph_manager.set_kv_cache(
                        layerwise_k_caches, layerwise_v_caches
                    )
                else:
                    start_cache_num = self.layerwise.load_list.index(
                        self.layerwise.edge_start_layer_count
                    ...
                    self.graph_manager.set_kv_cache(
                        k_caches_sp2, v_caches_sp2, specified_kv_caches
                    )
            else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        logits = self.execute_ascend_operator(
            acl_inputs, acl_param, is_prefill, **kwargs
        )
        return logits
    ...
    def _update_matmul_params(self, quantize: QuantType):
        ...
        is_aclnn_qmm = quantize in (
            QuantType.W8A8,
            QuantType.W8A8_DYNAMIC,
            QuantType.W8A8_PDMIX,
        )

        if self.soc_info.soc_version in a2_socs + a3_socs and (
            is_float or is_aclnn_qmm
        ):
            self.soc_info.matmul_nd_nz = True

            # in (A2 + fp16 + float) or aclnn_qmm cases, using aclnn backend
            if (
                self.soc_info.soc_version in a2_socs
                and is_float
